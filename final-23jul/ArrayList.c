...
/** \brief nueva objects of list, use compare pFunc
 * \param pList ArrayList* Pointer to arrayList
 * \param pFunc (*pFunc) Pointer to fuction to compare elements of arrayList
 * \param order int  [1] indicate UP - [0] indicate DOWN
 * \return int Return (-1) if Error [pList or pFunc are NULL pointer]
 *                  - (0) if ok
 */
ArrayList* al_nueva(ArrayList* this, int (*pFunc)(void*, char*), char* texto)
{
    ArrayList* returnAux = NULL;
    ...
    for(int i=0; i<tam; i++)
    {
        aux=al_get(this, i);
                    ...
                    printf("%s--", texto);
printf("%s\n", ((Reparto*)aux)->localidad);

        if(pFunc(aux,texto))
        {
            printf("hola: %s", ((Reparto*)aux)->localidad);
