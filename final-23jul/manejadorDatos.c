...
int md_cargar_Entrega(ArrayList* this, char nombreArchivo[])
{
            ...
            do
            {
               // aux = (Est1*) malloc(sizeof(Est1));//le asigna una dire de memoria cada vez que entra al while
                ...
                if(r==5)
                {
                   ...
                   aux = rep_SeterDatos((atoi(var1)),var2,var3,var4,var5);
...
int md_guardar_Entrega(ArrayList* this, char nombrearchivo[])
    {
        int retorno=-1;
        if(this!=NULL && !(al_isEmpty(this)))
        {
            ...
            else
            {
                ...
                for (int i=0; i<len; i++)
                {
                    aux = (Reparto*)al_get(this, i);

                    printf("%d,%s,%s,%s,%s\n",rep_get_id(aux),rep_get_producto(aux),rep_get_direccion(aux),rep_get_localidad(aux),rep_get_recibe(aux));
